...
class DeployerBasicMode {
    ...
    */
     ...
     */
    async loadLogic(name, scTmplParams) {
     ...
     */
    async optInAccountToASA(asa, accountName, flags) {
     ...
     * @param flags Transaction flags
     */
    async optInLsigToASA(asa, lsig, flags) {
        this.assertCPNotDeleted({
            type: web_1.types.TransactionType.OptInASA,
...
class DeployerDeployMode extends DeployerBasicMode {
     ...
     * @param scTmplParams: Smart contract template parameters (used only when compiling PyTEAL to TEAL)
     */
    async fundLsig(name, flags, payFlags, scTmplParams) {
        try {
     ...
     */
    async _mkLsig(name, scTmplParams, signer) {
        this.assertNoAsset(name);
        let lsigInfo = {};
        try {
            const lsig = await (0, lsig_1.getLsig)(name, this.algoOp.algodClient, scTmplParams);
            if (signer) {
                lsig.sign(signer.sk);
                lsigInfo = {
                    creator: signer.addr,
     ...
     *     (used only when compiling PyTEAL to TEAL)
     */
    async mkDelegatedLsig(name, signer, scTmplParams) {
     ...
     * @param scTmplParams: Smart contract template parameters (used only when compiling PyTEAL to TEAL)
     */
    async mkContractLsig(name, scTmplParams) {
     ...
     */
    async deployApp(approvalProgram, clearProgram, flags, payFlags, scTmplParams, appName) {
        const name = appName ?? (approvalProgram + "-" + clearProgram);
        this.assertNoAsset(name);
        let sscInfo = {};
        try {
            sscInfo = await this.algoOp.deployApp(approvalProgram, clearProgram, flags, payFlags, this.txWriter, scTmplParams);
        }
        catch (error) {
            this.persistCP();
            console.log(error);
     ...
     * the checkpoint "key", and app information will be stored agaisnt this name
     */
    async updateApp(sender, payFlags, appID, newApprovalProgram, newClearProgram, flags, scTmplParams, appName) {
        this.assertCPNotDeleted({
        ...
        let sscInfo = {};
        try {
            sscInfo = await this.algoOp.updateApp(sender, payFlags, appID, newApprovalProgram, newClearProgram, flags, this.txWriter, scTmplParams);
        }
        catch (error) {
            this.persistCP();
            console.log(error);
...
class DeployerRunMode extends DeployerBasicMode {
    ...
    async fundLsig(_name, _flags, _payFlags, _scInitParams) {
        throw new web_1.BuilderError(web_1.ERRORS.BUILTIN_TASKS.DEPLOYER_EDIT_OUTSIDE_DEPLOY, {
            methodName: "fundLsig"
        });
    }
    async mkDelegatedLsig(_name, _signer, _scInitParams) {
        throw new web_1.BuilderError(web_1.ERRORS.BUILTIN_TASKS.DEPLOYER_EDIT_OUTSIDE_DEPLOY, {
            methodName: "delegatedLsig"
        });
    }
    async deployApp(approvalProgram, clearProgram, flags, payFlags, scInitParam, appName) {
        throw new web_1.BuilderError(web_1.ERRORS.BUILTIN_TASKS.DEPLOYER_EDIT_OUTSIDE_DEPLOY, {
     ...
     *     (used only when compiling PyTEAL to TEAL)
     */
    async updateApp(sender, payFlags, appID, newApprovalProgram, newClearProgram, flags, scTmplParams) {
        this.assertCPNotDeleted({
            type: web_1.types.TransactionType.UpdateApp,
            sign: web_1.types.SignType.SecretKey,
            fromAccount: sender,
        ...
        return await this.algoOp.updateApp(sender, payFlags, appID, newApprovalProgram, newClearProgram, flags, this.txWriter, scTmplParams);
    }
}
